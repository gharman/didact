(ns didact.gp
  ...
  (:require [didact.functions :as f]
            [incanter.stats :as i-stats :only (levenshtein-distance)]))
...
(defmacro def-fitness-function [difference-function hit-function]
  "Create a fitness function. It will use the given functions to determine
   a numerical difference between a target value (i.e. from a fitness case)
   and an actual value generated by an evolved program and to determine
   (boolean) whether a given program is a hit."
  `(fn [~'program ~'fitness-case]
     (let [~'numargs (- (count ~'fitness-case) 1) ; -1 b/c first is the target value, not an arg
           ~'target-value (first ~'fitness-case)
           ~'actual-value (apply ~'program (rest ~'fitness-case))
           ~'sf (~difference-function ~'target-value ~'actual-value)
           ~'hit (~hit-function ~'target-value ~'actual-value)]
       {:hit? ~'hit
        :standardized-fitness ~'sf})))

;;; Fitness function implementations for common data types
(def fitness-function-number-default
  (def-fitness-function
    (fn [target-value actual-value] ; Difference function
      (f/abs (- target-value actual-value))) ; Simple value differential
    (fn [target-value actual-value] ; Hits function
      (= target-value actual-value)))) ; Simple equality

(def fitness-function-string-default
  (def-fitness-function
    (fn [target-value actual-value] ; Difference function
      (i-stats/levenshtein-distance target-value actual-value)) ; Diff = number of edits
    (fn [target-value actual-value] ; Hits function
      (= target-value actual-value)))) ; Equality
;; TODO seems like overkill to load in all of incanter just for the levenshtein function - maybe just implement that one function here...

(def fitness-function-boolean-default
  (def-fitness-function
    (fn [target-value actual-value] ; Difference function
